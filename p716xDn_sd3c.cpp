...
////////////////////////////////////////////////////////////////////////////////
p716xDn_sd3c::p716xDn_sd3c(p716x_sd3c * p716xSd3cPtr, int chanId, 
        ...
        _dataInterruptPeriod(0.0)
{
    ...
    case p716x_sd3c::DDC10DECIMATE:
        if (_isBurst) {
            ...
            if (_chanId > 1) {
                ELOG << "Normal sampling requested for channel " << _chanId <<
                        ", but it's only supported on 0 or 1";
                raise(SIGINT);
            }
        }
        break;
    ...
    default:
        ...
        if (_isBurst) {
            ELOG << "Burst sampling requested for channel " << _chanId <<
                    ", but " << p716x_sd3c::ddcTypeName(_sd3c.ddcType()) <<
                    " does not support it!";
            raise(SIGINT);
        }
        break;
...
int p716xDn_sd3c::filterSetup() {
    ...
    if (_gaussianFile.size() != 0) {
        ...
        case p716x_sd3c::DDC4DECIMATE: {
            ...
            int fwidth_ns = (int)(
            		1.0e9 * _sd3c.countsToTime(_sd3c._timerWidth(p716x_sd3c::TX_PULSE_TIMER))/_sd3c.codeLength()
            		);
			...
			default:
				ELOG << "chip width specification of "
					  ...
					  << " (" << _sd3c.countsToTime(_sd3c._timerWidth(p716x_sd3c::TX_PULSE_TIMER))/_sd3c.codeLength()
					  << "s) is not recognized\n";
				raise(SIGINT);
			break;
            }
        ...
        case p716x_sd3c::DDC10DECIMATE: {    // pulse_width in 50 MHz counts
            pulsewidthUs = 0.5;
...
//////////////////////////////////////////////////////////////////////////////////
char*
p716xDn_sd3c::ptBeam(char* pulseTag, char* metadata) {
    boost::recursive_mutex::scoped_lock guard(_mutex);
    ...
    while(1) {
        ...
        syncHuntMsg << "Sync hunt words on card: " << _p716x.getCardIndex()
                    << ", chan: " << _chanId << " : ";
...
//////////////////////////////////////////////////////////////////////////////////
void
p716xDn_sd3c::unpackPtChannelAndPulse(const char* buf, unsigned int & chan,
        unsigned int & pulseNum) {
...
//////////////////////////////////////////////////////////////////////////////////
void
p716xDn_sd3c::unpackPtMetadata(const char* buf, float & angle1,
        float & angle2, bool & xmitPolHorizontal) {
